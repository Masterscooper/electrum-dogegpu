...
APPIMAGE="$DISTDIR/electrum-dogegpu-$VERSION-x86_64.AppImage"
...
"$python" -m pip install --no-build-isolation --no-dependencies --no-binary :all: --no-warn-script-location \
    --cache-dir "$PIP_CACHE_DIR" -r "$CONTRIB/deterministic-build/requirements-dogegpu-pypi.txt"
"$python" -m pip install --no-build-isolation --no-dependencies --no-binary :all: --no-warn-script-location \
    --cache-dir "$PIP_CACHE_DIR" -r "$CONTRIB/deterministic-build/requirements-dogegpu-git.txt"
...
cp "$PROJECT_ROOT/electrum-dogegpu.desktop" "$APPDIR/electrum-dogegpu.desktop"
