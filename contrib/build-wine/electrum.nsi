...
;Variables
  ...
  !define PRODUCT_NAME "Electrum DogeGPU"
  !define PRODUCT_WEB_SITE "https://github.com/Masterscooper/electrum-dogegpu"
  !define PRODUCT_PUBLISHER "The DogeGPU Community"
...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "dist/electrum-dogegpu-setup.exe"
...
Section
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-dogegpu-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-dogegpu-${PRODUCT_VERSION}.exe" "" "$INSTDIR\electrum-dogegpu-${PRODUCT_VERSION}.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME} Testnet.lnk" "$INSTDIR\electrum-dogegpu-${PRODUCT_VERSION}.exe" "--testnet" "$INSTDIR\electrum-dogegpu-${PRODUCT_VERSION}.exe" 0


  ;Links bitcoin: and lightning: URIs to Electrum
  WriteRegStr HKCU "Software\Classes\doge" "" "URL:doge Protocol"
  WriteRegStr HKCU "Software\Classes\doge" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\doge" "DefaultIcon" "$\"$INSTDIR\electrum.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\doge\shell\open\command" "" "$\"$INSTDIR\electrum-dogegpu-${PRODUCT_VERSION}.exe$\" $\"%1$\""
  ...
  ;WriteRegStr HKCU "Software\Classes\lightning\shell\open\command" "" "$\"$INSTDIR\electrum-dogegpu-${PRODUCT_VERSION}.exe$\" $\"%1$\""
